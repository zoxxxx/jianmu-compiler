...
void translate_main(CodeGen *codegen) {
    ...
    offset_map["%op3"] = -84; 
    // 先获得 %op1 的值, 然后获得 %op1 指向的空间的值, 最后将这个值写入
    // %op3 对应的内存空间中
    codegen->append_inst("ld.d",
                         {"$t0", "$fp", std::to_string(offset_map["%op1"])});
    ...
    offset_map["%op4"] = -88;
    // 先获得 %op3 的值, 然后将其乘以 3, 最后将结果写入 %op4 对应的内存空
    // 间中
    codegen->append_inst("ld.w",
                         {"$t0", "$fp", std::to_string(offset_map["%op3"])});
    codegen->append_inst("addi.w",
                         {"$t1", "$zero", "3"});
    codegen->append_inst("mul.d",
                         {"$t1", "$t0", "$t1"});
    codegen->append_inst("st.w",
    ...
    offset_map["%op5"] = -92; 
    // 先获得 %op2 的值, 然后获得 %op2 指向的空间的值, 最后将这个值写入
    // %op5 对应的内存空间中
    codegen->append_inst("ld.d",
